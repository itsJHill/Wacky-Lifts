...
extension WorkoutTemplate {
    struct Exercise: Identifiable, Hashable, Codable, Sendable {
        ...
        private static func resolveOrCreateExercise(name: String, machineId: UUID?) -> UUID {
            if let existing = ExerciseStore.shared.sortedByName().first(where: {
                $0.name.localizedCaseInsensitiveCompare(name) == .orderedSame
            }) {
                return existing.id
            }
            let newExercise = ExerciseDefinition(name: name, machineId: machineId)
            ExerciseStore.shared.add(newExercise)
